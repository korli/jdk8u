...
ifeq ($(findstring true, $(JVM_VARIANT_ZERO) $(JVM_VARIANT_ZEROSHARK)), true)
  ...
  PLATFORM         = haiku-zero
  VM_PLATFORM      = haiku_$(subst i386,i486,$(ZERO_LIBARCH))
  HS_ARCH          = zero
  ARCH             = zero
endif
...
ifeq ($(ARCH), sparc64)
  ifeq ($(ARCH_DATA_MODEL), 64)
    ARCH_DATA_MODEL  = 64
    MAKE_ARGS        += LP64=1
    PLATFORM         = haiku-sparcv9
    VM_PLATFORM      = haiku_sparcv9
  else
    ARCH_DATA_MODEL  = 32
    PLATFORM         = haiku-sparc
    VM_PLATFORM      = haiku_sparc
  endif
  HS_ARCH            = sparc
endif
...
ifneq (,$(findstring $(ARCH), amd64 x86_64))
  ifeq ($(ARCH_DATA_MODEL), 64)
    ARCH_DATA_MODEL = 64
    MAKE_ARGS       += LP64=1
    PLATFORM        = haiku-amd64
    VM_PLATFORM     = haiku_amd64
    HS_ARCH         = x86
  else
    ARCH_DATA_MODEL = 32
    PLATFORM        = haiku-i586
    VM_PLATFORM     = haiku_i486
    HS_ARCH         = x86
...
ifneq (,$(findstring $(ARCH), BePC i686 i586))
  ARCH_DATA_MODEL  = 32
  PLATFORM         = haiku-i586
  VM_PLATFORM      = haiku_i486
  HS_ARCH          = x86
...
ifeq ($(ARCH), ppc64)
  ARCH_DATA_MODEL  = 64
  MAKE_ARGS        += LP64=1
  PLATFORM         = haiku-ppc64
  VM_PLATFORM      = haiku_ppc64
  HS_ARCH          = ppc
endif
...
# No SA Support for x86, SPARC, PPC, IA64, ARM or zero
ADD_SA_BINARIES/x86   =
...
-include $(HS_ALT_MAKE)/haiku/makefiles/defs.make
